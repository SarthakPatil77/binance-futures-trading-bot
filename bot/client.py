...
class BinanceClient:
    ...
    def _request(self, method: str, endpoint: str, params: dict = None, signed: bool = True):
        ...
        logger.debug(f"REQUEST {method} {url} | params: { {k: v for k, v in params.items() if k != 'signature'} }")
        ...
        try:
            ...
            logger.debug(f"RESPONSE HTTP {response.status_code} | body: {response.text[:500]}")
